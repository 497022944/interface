...
class xianshang_name:
    def xiashang_names(self):
        global common_return_a,xubao
        ...
        for i in range(len(cict1)):
            for j in range(i * num, num * (i + 1)):
                ...
                if len(licenseNO) > 12 and len(licenseNO) < 18:
                    ...
                    xianshangbaojiajieguo1['续保结果'] = StatusMessage
                ...
                if common_return_a == True:
                    ...
                    if xubaovalues >= 0 and xubaovalues <= 10:
                        ...
                        StatusMessage = xubao.res["StatusMessage"]
                        common_return_a = common_return.testxubao(self, StatusMessage)
                        log_info_value.update(车五项信息车牌=xubao.res['UserInfo']['LicenseNo'],
                                              发动机=xubao.res['UserInfo']['EngineNo'],
                                              品牌=xubao.res['UserInfo']['ModleName'], 车架=xubao.res['UserInfo']['CarVin'],
                                              注册日期=xubao.res['UserInfo']['RegisterDate'],
                                              上年投保=xubao.res['SaveQuote']['Source'])
                        log.log_info(self, current_entry=log_info_value)
                        if common_return_a == True:
                            ...
                            if results1 == True:
                                ...
                                log_info_value.update(车牌号或车架号=licenseNO, 车损=CheSun, 车损不计免赔=CheSunBjm, 三者=SanZhe,
