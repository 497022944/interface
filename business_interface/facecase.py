...
class Run_class:
    ...
    def test_02_Basic_double_coverage(self):
        ...
        if Online == 1:
            ...
            # licenseNO = licenseNo.split(',')
            # LicenseNO = licenseNO
            for i in range(len(cict1)):
                for j in range(i*num, num*(i+1)):
                    licenseNO = licenseNo1[j]
                    cict = cict1[i]
                    #print(licenseNo1[j])
                    log_info_value.update(选择城市=cict1[i], 车牌号=licenseNo1[j])
                    log.log_bug(self, log_info_value)
                    ...
                    if common_return_a == True:
                        ...
                        if xubaovalues >= 0 and xubaovalues <=10:
                            log_info_value.update(licenseNo=licenseNo1[j], 续保耗时=xubaovalues)
                            log.log_info(self, current_entry=log_info_value)
                            log_info_value.update(续保结果=xubao.res, 车牌=licenseNo1[j])
                            log.log_info(self, current_entry=log_info_value)
                            StatusMessage = xubao.res["StatusMessage"]
                            common_return_a = common_return.testxubao(self, StatusMessage)
                            ...
                            if common_return_a == True:
                                ...
                                log_bug_value.update(车牌=licenseNo1[j], 报价耗时=baojavalues)
                                log.log_bug(self, log_bug_value)
                                log_info_value.update(车牌号=licenseNo1[j], 关系人信息车主=personnel)
                                log.log_info(self, log_info_value)
                                ...
                                if results1 == True:
                                    ...
                                    log_info_value.update(车牌=licenseNo1[j], 车损=CheSun, 车损不计免赔=CheSunBjm, 三者=SanZhe, 三者不计免赔=SanZheBjm, 司机=SiJi, 司机不计免赔=SiJiBjm, 乘客=ChengKe, 乘客不计免赔=ChengKeBjm)
                                    log.log_info(self, log_info_value)
                                    ##QuoteStatuss报价状态，-1=未报价， 0=报价失败，>0报价成功
                                    if QuoteResultt1 <= 0:
                                        log_info_value.update(报价结果=QuoteResultt)
                                        if Repeat_insurance == True:
                                            success1 = success.append(licenseNo1[j])
                                            log_info_value.update(商业=BizTotal, 车船=TaxTotal, 交强=ForceTotal, 车牌=licenseNo1[j],报价结果=Repeat_insurance)
                                            log.log_info(self, log_info_value)
                                        elif the_same == True:
                                            success1 = success.append(licenseNo1[j])
                                            log_info_value.update(商业=BizTotal, 车船=TaxTotal, 交强=ForceTotal, 车牌=licenseNo1[j],报价结果=the_same)
                                            log.log_info(self, log_info_value)
                                        else:
                                            defeated1 = defeated.append(licenseNo1[j])
                                            log_info_value.update(车牌=licenseNo1[j], 报价结果=QuoteResultt)
                                            log.log_bug(self, log_info_value)
                                    else:
                                        log_info_value.update(报价结果=QuoteResultt)
                                        log_info_value.update(商业=BizTotal, 车船=TaxTotal, 交强=ForceTotal, 车牌=licenseNo1[j], 报价结果=QuoteResultt)
                                        success1 = success.append(licenseNo1[j])
                                        log.log_info(self, log_info_value)
                                    ...
                                    defeated1 = defeated.append(licenseNo1[j])
                                    log_bug_value.update(车牌=licenseNo1[j], 异常结果=StatusMessage)
                                    log.log_bug(self, log_bug_value)
                                continue
                            ...
                            defeated1 = defeated.append(licenseNo1[j])
                            log_bug_value.update(车牌=licenseNo1[j], 续保耗时=xubaovalues)
                            log.log_bug(self, current_entry_bug=log_bug_value)
                            continue
                    else:
                        defeated1 = defeated.append(licenseNo1[j])
                        log_bug_value.update(车牌=licenseNo1[j], 续保耗时=xubaovalues, 续保异常=StatusMessage)
                        log.log_bug(self, log_bug_value)
                        continue
        else:
